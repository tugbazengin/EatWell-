...
struct ProfileView: View {
    ...
    var body: some View {
        NavigationStack {
            BaseView(title: "Profil", showsScrollView: true) {
                VStack(spacing: 20) {
                    ...
                    VStack(spacing: 15) {
                        ...
                        ProfileInfoCard(title: "Günlük Kalori İhtiyacı", value: "\(viewModel.profile.dailyCalories) kcal", icon: "flame.fill", color: .red)
                        ProfileInfoCard(title: "Günlük Su Tüketimi", value: "\(viewModel.profile.dailyWater) L", icon: "drop.fill", color: .blue)
                    }
                    .padding(.horizontal)
